...
| 기능 | v1.0.x | v1.1.x |
|------|--------|--------|
| **모델 수** | 1개 (Llama) | 2개 (Llama + Bllossom) |
| **응답 방식** | 동기/스트리밍 | 스트리밍 전용 |
| **복잡도 분석** | ✅ BART 모델 | ❌ 제거됨 |
| **캐릭터 시스템** | ❌ 없음 | ✅ 10개 파라미터 |
| **검색 연동** | ❌ 없음 | ✅ Google Search |
| **환경** | 네이티브 | Docker 권장 |
| **토큰 할당** | 적응형 (100-500) | 고정 (512) |

### 마이그레이션 가이드

1. **API 엔드포인트** 변경: `/Llama` → `/Llama_stream`
2. **응답 처리** 변경: JSON → 스트리밍 텍스트
3. **GPU 할당** 재구성: 듀얼 GPU 설정 필요
4. **환경 변수** 추가: 검색 API 키 설정
...
## 라이선스
이 소프트웨어는 Meta의 Llama 모델 라이선스와 MLP-KTLim의 Korean Bllossom 모델 라이선스를 준수합니다.
